...
func NewEncryptor(config *EncryptionConfig) (*Encryptor, error) {
	if config == nil {
		return nil, errors.New("encryption config cannot be nil")
	}

	// Validate key length
	keyLen := len(config.Key)
	if keyLen != 16 && keyLen != 24 && keyLen != 32 {
		return nil, errors.New("encryption key must be 16, 24, or 32 bytes (use GenerateAESKey() to generate a valid key)")
	}
...
func (e *Encryptor) Decrypt(encodedCiphertext string) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(err, "failed to decrypt: invalid ciphertext or key")
	}
...
func (e *Encryptor) DecryptBytes(ciphertext []byte) ([]byte, error) {
	...
	plaintext, err := gcm.Open(nil, nonce, ciphertext, nil)
	if err != nil {
		return nil, errors.Wrap(err, "failed to decrypt: invalid ciphertext or key")
	}
